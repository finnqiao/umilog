...
final class MapUIReducerTests: XCTestCase {
    ...
    func testDrillDownToRegion() {
        ...
        XCTAssertEqual(ctx.hierarchyLevel, .region("Caribbean"))
    }

    func testDrillDownToArea() {
        let state = MapUIMode.explore(ExploreContext(hierarchyLevel: .region("Caribbean")))
        let result = MapUIReducer.reduce(
            state: state,
    ...
    func testOpenSiteInspection() {
        let exploreCtx = ExploreContext(
            hierarchyLevel: .region("Caribbean"),
            filterLens: .saved
        )
        ...
        XCTAssertEqual(ctx.returnContext.hierarchyLevel, .region("Caribbean"))
    ...
    func testOpenFilter() {
        let exploreCtx = ExploreContext(hierarchyLevel: .region("Caribbean"))
        let state = MapUIMode.explore(exploreCtx)
        let result = MapUIReducer.reduce(
            state: state,
            action: .openFilter,
    ...
    func testCloseFilterApply() {
        let returnContext = ExploreContext(hierarchyLevel: .region("Caribbean"))
